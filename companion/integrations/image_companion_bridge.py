...
class ImageCompanionBridgeMixin:
    ...
    def _image_read_reference_batch_sync(paths: tuple[str, ...]) -> tuple[bytes, ...]:
        ...
        for path in paths:
            descriptor = -1
            try:
                # POSIX uses O_NOFOLLOW for the race-resistant check. Windows
                # does not expose it; reject an already-linked path before
                # opening so normal regular files remain usable there while
                # preserving the stronger native check where available.
                if nofollow is None and os.path.islink(path):
                    raise _ImageCurrentContractError("reference_asset_symlink")
                ...
                if (
                    ...
                    or (os.name != "nt" and before.st_mtime_ns != after.st_mtime_ns)
                ):
    ...
    async def _image_companion_test_endpoint(
        ...
        if mode == "incompatible" and api is not None:
            return {
                "ok": False,
                "unsupported": True,
                "test_status": "unsupported",
                "code": "image_contract_incompatible",
                "message": "已检测到 Image 扩展，但当前运行实例未通过正式 Companion 契约。",
                "detail": f"旧式单端点测试未执行（{reason or 'contract_incompatible'}）。请更新 Image 扩展后重试。",
                "next_step": "更新并重载 astrbot_plugin_image_companion，再运行完整图片生成链路测试。",
            }
        if mode == "missing":
